...
public class InventoryItem : MonoBehaviour
{
    ...
    public void HandleAction(string action)
    {
        ...
        if (action == "Pick Flower")
        {
            ...
            manager.player.flowchart.ExecuteBlock("BlueFlower");
        }

        if(action == "Extract Minerals")
        {
            manager.player.timer.AddTime(quant);
            manager.player.inventory.AddItem("Mineral (g)", 5);
            manager.player.flowchart.ExecuteBlock("extract minerals");
        }
    }
